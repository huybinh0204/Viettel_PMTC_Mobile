...
import global from '../../features/Invoice/global'

class HeaderForInvoice extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  _updateHeader = () => {
    this.setState({
      isHideAllGeneral: global.isHideGeneralInfor || global.isHideMoneyInfor || global.isHideManagermentInfor,
      isHideAllDetail: global.isHideDetailGeneralInfor || global.isHideDetailMoneyInfor || global.isHideDetailOtherInfor,
    })
  }
          ...
          <View style={styles.wrapper}>
            {
              ((indexOfTab === 0) || (indexOfTab === 1 && typeOfIconTab === 1)) && !disableEye && (
                <TouchableOpacity
                  style={[styles.button]}
                  onPress={() => {
                    let { isHideAllGeneral, isHideAllDetail } = this.state
                    if (indexOfTab === 0) {
                      global.hideMoneyInfor(isHideAllGeneral)
                      global.hideGeneralInfor(isHideAllGeneral)
                      global.hideManagermentInfor(isHideAllGeneral)
                      this.setState({ isHideAllGeneral: !isHideAllGeneral })
                    } else {
                      global.hideDetailGeneralInfor(isHideAllDetail)
                      global.hideDetailMoneyInfor(isHideAllDetail)
                      global.hideDetailOtherInfor(isHideAllDetail)
...
export default connect(mapStateToProps, {})(HeaderForInvoice)
