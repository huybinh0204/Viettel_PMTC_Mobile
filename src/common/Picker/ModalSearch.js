...
import { View, Text, Modal, StyleSheet, TouchableOpacity, TouchableWithoutFeedback, TextInput, Platform } from 'react-native';
import Autocomplete from 'react-native-autocomplete-input';
import AntDesign from 'react-native-vector-icons/AntDesign'
import { HEIGHT, WIDTH, WIDTHXD, getFontXD, HEIGHTXD } from '../../config/Function';
                ...
                <View style={styles.container}>
                  <Autocomplete
                    ...
                    renderTextInput={(iProps) => (
                      <View style={{
                        ...
                        <AntDesign name="search1" size={WIDTHXD(50)} color="#77869E" />
                        <TextInput
                          ref={ref => this.refList.onRefTextInput(ref)}
                    ...
                    renderItem={({ item, index }) => {
                      let evenRow = (index === (this.listSearch && (this.listSearch.length - 1)));
                      let highlighted = this.state.selectedIndex === index;
                      return (
                        <TouchableOpacity
                          key={index.toString()}
                          style={[styles.itemStyle, highlighted && { backgroundColor: '#e3e8f2' }]}
                          onPress={() => {
                            onValueChange && onValueChange(item.text ? `${item.text}` : `${item.name}`, item)
                            this.setState({ query: item.text ? item.text : item.name, selectedIndex: index })
                            this.setModalVisible(false)
                          }}
                        >
                          <Text style={styles.itemText}>
                            {item.text ? item.text && `${item.text}` : item.name && `${item.name}`}
                          </Text>
                        </TouchableOpacity>
                      )
                    }}
                    flatListProps={{
                      ItemSeparatorComponent: () => (<View style={{ width: '100%', height: 0.5, backgroundColor: R.colors.borderD4 }} />),
                      ListEmptyComponent: () => (<Text style={styles.itemText}>Không có nội dung</Text>)
                    }}
                  />
                  {this.state.query.length !== 0
                    && (
                      ...
                      >
                        <AntDesign name="close" size={WIDTHXD(40)} color="#BAC7D3" />
                      </TouchableOpacity>
                    )
                  }
                </View>
