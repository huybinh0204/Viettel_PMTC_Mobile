...
import { HEIGHTXD, WIDTHXD, getFontXD } from '../../config/Function';
...
class PickerSearch extends Component {
  ...
  render() {
    ...
    const { textStyle, containerStyle, placeholder, width, data, title, onValueChange, findData, disabled, height, tempDisabled, onShowPorm, style } = this.props
    const choosed = (this.props.value != '' && this.props.value !== null) ? true : false;

    return (
      <View style={[styles.container, style ? style : {}]}>
        <TouchableOpacity
          disabled={disabled}
          onPress={() => { tempDisabled ? (onShowPorm && onShowPorm()) : this.ModalSearch.setModalVisible(true) }}
          style={[styles.searchPicker, containerStyle && containerStyle, width && { width }, height && { height }]}
        >
          {
            (value === '' && this.props.value && this.props.value === '') ? <Text>{placeholder && placeholder}</Text>
              : <Text numberOfLines={1} style={[styles.textStyle, textStyle && textStyle, width && { width: width - WIDTHXD(150) }]}>{this.props.value && this.props.value}</Text>
          }
          ...
          <TouchableOpacity
            ...
            <AntDesign name={(!choosed || disabled) ? 'search1' : 'close'} size={WIDTHXD(43)} color={R.colors.iconGray} />
          </TouchableOpacity>
        </TouchableOpacity>
