...
import { WIDTHXD, HEIGHTXD, getFontXD, getLineHeightXD, numberWithCommas } from '../../../../../config';
  ...
  return (
    <View style={styles.mainContainer}>
      <TouchableOpacity onPress={() => onPressItem && onPressItem()}
        onPressOut={() => props.onPressOut && props.onPressOut()}
        ...
        <View style={styles.containerItem}>
          <View style={{ flexDirection: 'row', marginTop: HEIGHTXD(24), justifyContent: 'space-between' }}>
            <Text style={[styles.textNameInvoice, { flexWrap: 'wrap', flex: 1 }]} numberOfLines={2} ellipsizeMode='tail'>{nameInvoices && nameInvoices}</Text>
            <Text style={[styles.textNameInvoice, { alignItems: 'flex-end' }]}>{requestAmounts && numberWithCommas(requestAmounts)}</Text>
          </View>
          <View style={{ flexDirection: 'row', marginTop: HEIGHTXD(30), marginBottom: HEIGHTXD(41), }}>
            ...
            <Text style={styles.textQuantum}>{`${cUomIdss && cUomIdss} x VNĐ ${prices && numberWithCommas(prices)}`}</Text>
          </View>
        </View>
      </TouchableOpacity>
    </View>
