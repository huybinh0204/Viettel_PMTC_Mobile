...
class ListFileAttachment extends Component {
  ...
  fetch = async () => {
    try {
      if (!this.props.screenProps.idInvoice) {
  ...
  _onDel = async (_, indexOfItem) => {
    // request remove attachment from invoice
    try {
      const response = await invoice.removeAttachmment(this.state.data[indexOfItem].adAttachmentId);
  ...
  upload = async (file) => {
    try {
      ...
      const response = await axios.post(`http://222.252.22.174:8080/erp-service/adAttachmentServiceRest/attachFile/1000369/${this.props.screenProps.idInvoice}/1/${user_id}/${user_id}`, formData, {
        headers: {
  ...
  render() {
    ...
    let List = this.props.screenProps.tabIndex === 2 ? FlatListSwipe : FlatList;
