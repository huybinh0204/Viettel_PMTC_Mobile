...
export default (props) => {
  let { title, content, width, marginTop } = props
  return (
    <View style={[styles.flexColumn, marginTop && { marginTop: HEIGHTXD(30) }]}>
      <Text style={styles.label}>{title && title}</Text>
      <View style={[styles.wrapperText, width && { width }]}>
        <Text numberOfLines={1} style={styles.content}>{content && content}</Text>
      </View>
    </View>
  )
}
