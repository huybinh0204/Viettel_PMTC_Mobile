...
const ItemStatement = (props: Props) => {
  const { time, color, content, onPressItem } = props;
  ...
  const payStatus = content && content.payStatus ? (content.payStatus === 'Chưa chi' ? '' : content.payStatus) : '';

  return (
    <View style={styles.item}>
      <TouchableOpacity style={styles.content} onPress={() => onPressItem(content.cAdvanceRequestId)}>
        ...
        <View style={styles.secView}>
          <View style={styles.leftTxt}>
            ...
            <Text style={[styles.price, { color }]}>{toPriceVnd(vwAmount)}</Text>
          </View>
