...
class ItemVOffice extends Component<Props, State> {
  ...
  render() {
    ...
    let signerstatus = ''
    if (body.signerstatus) {
      _.forEach(R.strings.local.TRANG_THAI_KY, item => {
        if (item.value === body.signerstatus) signerstatus = item.name
      })
    }
    return (
          ...
          >
            <View>
              <View style={styles.row}>
                <Text style={[styles.label, { flex: 1.2, fontSize: getFontXD(36) }]}>Số KH, VB trình ký</Text>
                <Text style={[styles.label, { flex: 2, paddingLeft: WIDTHXD(36), color: R.colors.black0 }]}>
                  {body.hardCopyInfo ? body.hardCopyInfo : ''}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={[styles.label, { flex: 1.2, fontSize: getFontXD(36) }]}>Trạng thái ký</Text>
                <Text style={[styles.label, { flex: 2, paddingLeft: WIDTHXD(36), color: R.colors.black0 }]}>
                  {signerstatus}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={[styles.label, { flex: 1.5, fontSize: getFontXD(36) }]}>Ngày duyệt VO</Text>
                <Text style={[styles.label, { flex: 1.5, paddingLeft: WIDTHXD(36), color: R.colors.black0 }]}>{body.actiondate ? body.actiondate : ''}</Text>
                <View style={{ justifyContent: 'space-around', flexDirection: 'row', flex: 2 }}>
                  <View style={styles.ctnCheckbox}>
                    <CheckBox
                      checked={checked}
                      size={WIDTH(30)}
                      color={checked ? R.colors.colorNameBottomMenu : R.colors.colorCheckBox}
