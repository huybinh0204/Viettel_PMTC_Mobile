...
class ItemStatus extends Component {
  ...
  componentDidMount() {
    if (this.props.statementInfo) {
      this.setState({ body: this.props.statementInfo })
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    {
      if(nextProps.statementInfo.docstatus !== this.setState.docstatus) 
      this.setState({docstatus : nextProps.statementInfo.docstatus})
      if(nextProps.statementInfo.approveStatus !== this.setState.approveStatus) 
      this.setState({approveStatus : nextProps.statementInfo.approveStatus})
      if(nextProps.statementInfo.paymentStatus !== this.setState.paymentStatus) 
      this.setState({paymentStatus : nextProps.statementInfo.paymentStatus})
    }
    this.setState({statusButtonCO: nextProps.statusButtonCO})
  }
  ...
  render() {
    ...
    const { docstatus, approveStatus, paymentStatus, statusButtonCO } = this.state;
    const docStatusTmp = docstatus ? (statusButtonCO ? R.strings.local.TRANG_THAI_TAI_LIEU[0].name : R.strings.local.TRANG_THAI_TAI_LIEU[1].name) : ''
    return (
        ...
        <View style={{ paddingHorizontal: WIDTHXD(32)}}>
            <View style={{ marginBottom:HEIGHTXD(40) }}>
              <View style={styles.row}>
                ...
                <Text style={[styles.label, { flex: 2, paddingLeft: WIDTHXD(36), fontSize: getFontXD(42), color:R.colors.black0  }]}>
                  {approveStatus ? R.strings.local.TRANG_THAI_DUYET[parseInt(approveStatus)].name : ''}
                </Text>
              </View>
              <View style={styles.row}>
