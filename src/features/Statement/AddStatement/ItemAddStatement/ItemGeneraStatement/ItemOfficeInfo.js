...
class ItemOfficeInfo extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    {
      if(nextProps.statementInfo.hardCopyInfo !== this.setState.hardCopyInfo) 
      this.setState({hardCopyInfo : nextProps.statementInfo.hardCopyInfo})
      if(nextProps.statementInfo.signerstatus !== this.setState.signerstatus) 
      this.setState({signerstatus : nextProps.statementInfo.signerstatus})
      if(nextProps.statementInfo.issignerrecord !== this.setState.issignerrecord) 
      this.setState({issignerrecord : nextProps.statementInfo.issignerrecord})
      if(nextProps.statementInfo.actiondate !== this.setState.actiondate) 
      this.setState({actiondate : nextProps.statementInfo.actiondate})
    }
  }
  ...
  render() {
    ...
    const { 
      ...
      signerstatus, 
      issignerrecord, 
      actiondate  } = this.state;
      const checkSignerrecord = this.state.issignerrecord === "Y" ? true: false
    return (
          ...
          >
            ...
            <View style={{ marginTop: HEIGHTXD(12) }}>
              ...
              <View style={styles.row}>
                ...
                <Text style={[styles.label, { flex: 2, paddingLeft: WIDTHXD(36), color: R.colors.black0, fontSize:getFontXD(42) }]}>
                  {signerstatus ? R.strings.local.TRANG_THAI_KY[signerstatus].name : ''}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={[styles.label, { flex: 1.5 }]}>Ngày duyệt VO</Text>
                <Text style={[styles.label, { flex: 1.5, paddingLeft: WIDTHXD(36), color: R.colors.black0, fontSize:getFontXD(42) }]}>{actiondate ? actiondate : ''}</Text>
