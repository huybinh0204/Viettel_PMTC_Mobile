...
import SubmissionInfoLine from './ItemDetailStatement/SubmissionInfoLine';
...
class SubmissionInfo extends Component {
  ...
  renderItem = ({ item, index }) => {
    let day = item.transDate
      ? moment(mapTimeVNToWorld(item.transDate)).format('D')
      : '';
    let month = item.transDate
      ? moment(mapTimeVNToWorld(item.transDate)).format('M')
      : '';
    let year = item.transDate
      ? moment(mapTimeVNToWorld(item.transDate)).format('YYYY')
      : '';
    let content = {
...
export default connect(mapStateToProps, { setListInvoice })(SubmissionInfo);
