...
export default class DetailPayInfo extends Component {
  ...
  render() {
    ...
    const documentName = body.apCashDocumentTypeName ? body.apCashDocumentTypeName : ''
    const documentNo = body.apInvoiceDocumentNo ? body.apInvoiceDocumentNo : ''
    const dateAcct = body.accountingDate ? body.accountingDate : ''
    const arCashLineId = body.arCashLineId ? body.arCashLineId : ''
    const amtAcct = body.amtAcct ? body.amtAcct : ''
    const cCurrencyName = body.amount ? body.amount.toString() : ''
    const accountNo = body.accountNo ? body.accountNo : ''
    const descriptionLine = body.description ? body.description : ''
    let status = ''
    if (body.status) {
